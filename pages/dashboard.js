        ...
        <div className="max-w-5xl mx-auto space-y-10">
          ...
          <section className="grid md:grid-cols-3 gap-8">
            {features.map(({ title, slug, icon: Icon, desc }) => (
              <div
                key={slug}
                className="bg-black/80 rounded-2xl shadow-xl p-8 text-center border border-purple-700"
              >
                <Icon className="w-16 h-16 mb-4 text-purple-300 mx-auto" />
                <h2 className="text-xl font-bold text-white mb-2">{title}</h2>
                <p className="text-gray-400 mb-6">{desc}</p>
                <button
                  className="w-full mb-2 py-2 rounded-full bg-gradient-to-br from-purple-600 to-purple-800 text-white font-semibold hover:from-purple-500 hover:to-purple-700 transition"
                >
                  Preview
                </button>
                <button
